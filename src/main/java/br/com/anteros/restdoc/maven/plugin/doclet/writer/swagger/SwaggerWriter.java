...
import br.com.anteros.restdoc.maven.plugin.doclet.model.*;
import br.com.anteros.restdoc.maven.plugin.doclet.writer.Writer;
import br.com.anteros.restdoc.maven.plugin.doclet.writer.swagger.model.*;
...
import static br.com.anteros.restdoc.maven.plugin.doclet.writer.swagger.TypeUtils.*;
...
public class SwaggerWriter implements Writer {
        ...
        for (Endpoint endpoint : endpoints) {
            ...
            operations.add(
                    new Operation(
                            ...
                            dataType(endpoint.getTypeMirror()),
    ...
    private static Parameter getParameter(PathVar pathVar) {
        return new Parameter(
                ...
                basicType(pathVar.getTypeMirror()),
                null,
                true,
                false,
                allowableValues(pathVar.getTypeMirror())
        );
    }
    ...
    private static Parameter getParameter(QueryParam queryParam) {
        ...
        boolean container = isContainer(queryParam.getTypeMirror());

        return new Parameter(
                "query",
                queryParam.getName(),
                queryParam.getDescription(),
                (container ? internalContainerType(queryParam.getTypeMirror()) : basicType(queryParam.getTypeMirror())),
                null,
                queryParam.isRequired(),
                container,
                allowableValues(queryParam.getTypeMirror())
        );
    }
    ...
    private static Parameter getParameter(RequestBody requestBody) {
        return new Parameter(
                ...
                dataType(requestBody.getTypeMirror()),
                null,
                true,
                false,
                allowableValues(requestBody.getTypeMirror())
        );
    }
